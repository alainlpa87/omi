<div class="portlet col-md-12 portlet-new" id="container_{{$currentRequest->id}}"  data-id = "{{$project->id}}" data-request-id="{{$currentRequest->id}}" data-lead-id="{{$project->lead->id}}" data-date-action="{{$project->actionDate}}">
    <div class="box call">
        <div class="portlet-title">
            ...
            <ul class="nav nav-tabs" role="tablist">
                <li>
                    <a href="#portlet_tab_{{$currentRequest->id}}_4" data-toggle="tab" role="tab"> BUSINESS PROFILE </a>
                </li>
                <li>
                    <a href="#portlet_tab_{{$currentRequest->id}}_3" data-toggle="tab" role="tab"> PROJECT </a>
                </li>
                <li class="active">
                    <a href="#portlet_tab_{{$currentRequest->id}}_1" data-toggle="tab" role="tab"> CLIENT </a>
                </li>
            </ul>
        </div>
        <div class="portlet-body" id="portletBody_{{$project->id}}">
            <div class="tab-content col-md-12">
                <div class="tab-pane active" id="portlet_tab_{{$currentRequest->id}}_1" role="tabpanel">
                    <div class="col-md-12 vendorBox">
                        <div class="col-md-3">
                            <input type="checkbox" class="checkVendor" id="checkVendors_{{$currentRequest->id}}" data-projectId="{{$project->id}}" {{$project->vendor==1?"checked":""}}>
                            <span> Available for Vendors</span>
                        </div>
                        <div class="col-md-3">
                            <span>Payment:</span>
                            @if($project->lastContractPaidDate() != '0000-00-00 00:00:00' && $project->lastContractPaidDate() != '')
                                <span>{{date('m-d-Y',strtotime(str_replace('-','/',$project->lastContractPaidDate())))}}</span>
                            @else
                                <span>N/A</span>
                            @endif
                        </div>
                        ...
                        @if($project->totalPhase1AmountIn() != '')
                            <div class="col-md-2">
                                Phase1: {{money_format('%=#10.2n',$project->totalPhase1AmountIn())}}
                            </div>
                        @endif
                        @if($project->contractPPA() != '' && $project->contractPPA()->paid > 0)
                            <div class="col-md-2">
                                Phase2: {{money_format('%=#10.2n',$project->contractPPA()->paid)}}
                            </div>
                        @endif
                    </div>
                    <div class="col-md-12 boxProjectInfo">
                        <div class="col-md-3">
                            @if($currentRequest->created_at != '0000-00-00 00:00:00')
                                <p id="creation_p_{{$project->id}}">{{date('m-d-Y h:i:s',strtotime(str_replace('-','/',$currentRequest->created_at)))}}</p>
                            @else
                                <p id="creation_p_{{$project->id}}">{{$currentRequest->created_at}}</p>
                            @endif
                        </div>
                        <div class="col-md-4">
                            <i><strong><p id="request_p_{{$project->id}}">
                                        {{"REQUEST: ".$currentRequest->consultantStage}}
                                    </p></strong></i>
                        </div>
                        <div class="col-md-4">
                            ...
                            <i class="fa fa-folder-open fa-3 openFilesAdmin adminIcons" data-toggle="tooltip" data-placement="bottom" title="Open Files" data-id = "{{$project->id}}" ></i>
                            <i class="fa fa-upload fa-3 uploadFilesAdmin adminIcons" data-toggle="tooltip" data-placement="bottom" title="Upload Files" data-id = "{{$project->id}}" ></i>
                        </div>
                    </div>
                    <div class="col-md-12">
                ...
                <div class="tab-pane" id="portlet_tab_{{$currentRequest->id}}_3" role="tabpanel">
                    <div class="col-md-12">
                        ...
                        <div class="col-md-2 approveBox adminActions" data-requestid="{{$currentRequest->id}}" data-action="APPROVED">
                            <span class="spanRequestApprove"><i class="fa fa-thumbs-up fa-2x green"></i></span>
                        </div>
                        <div class="col-md-1">
                            <span></span>
                        </div>
                        <div class="col-md-2 approveNmiBox adminActions" data-requestid="{{$currentRequest->id}}" data-action="APPROVED BUT NMI">
                            <span class="spanRequestApproveNmi"><i class="fa fa-thumbs-up fa-2x"></i><i class="fa fa-question-circle fa-2x"></i></span>
                        </div>
                        <div class="col-md-1">
                            <span></span>
                        </div>
                        <div class="col-md-2 nmiBox adminActions" data-requestid="{{$currentRequest->id}}" data-action="NEEDS MORE INFO">
                            <span class="spanRequestNmi"><i class="fa fa-question-circle fa-2x dodgerblue"></i></span>
                        </div>
                        <div class="col-md-1">
                            <span></span>
                        </div>
                        <div class="col-md-2 rejectBox adminActions" data-requestid="{{$currentRequest->id}}" data-action="REJECTED">
                        ...
                        <h5>Admin Notes</h5>
                        <textarea class="adminNotes editableval col-md-12 col-xs-12" data-projectId="{{$project->id}}" data-requestId="{{$currentRequest->id}}">{{$currentRequest->adminNotes}}</textarea>
                    </div>
                </div>
                <div class="tab-pane" id="portlet_tab_{{$currentRequest->id}}_4" role="tabpanel">
