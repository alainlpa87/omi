...
<body onload="noBack();" onpageshow="if (event.persisted) noBack();" onunload="">
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    ...
                    <li>
                        <a href="{{route('launch')}}"><i class="fa fa-dashboard "></i>Launch Center</a>
                    </li>
                    <li>
                        <a href="{{route('launch/profile')}}"><i class="fa fa-user"></i>Inventor Profile</a>
                    </li>
                    <li>
                        ...
                        <ul class="nav nav-second-level">
                            @foreach($client->projects as $project)
                                @if($project->status!="EXCLUDE")
                                    <li>
                                        <a href="{{route('launch/project',$project->id)}}"><i class="fa fa-key "></i>{{$project->ideaName}}</a>
                                    </li>
                                @endif
                            @endforeach
                            @if(count($client->projects) == 0)
                                <li>
                                    <a href="{{route('sub')}}"><i class="fa fa-key "></i>Create New Project</a>
                                </li>
                            @endif
                        </ul>
                    </li>
                            ...
                            @foreach($client->projects as $project)
                                @if($project->status!="EXCLUDE")
                                    @if($project->lastContractNoWP() !=null && $project->lastContractNoWP()->paid < $project->lastContractNoWP()->price && in_array($project->stage,array("WOM", 'WALKTHRU',"DROP", "DROP/IMG", "ARCHIVE","PHASE2","CONTRACT")) && ($project->stage=="WALKTHRU"||$project->lastContractNoWP()->type=="IGUP"||$project->lastContractNoWP()->type=="PPA" || new \DateTime($project->actionDate) <= new \DateTime()))
                                        @if($project->lastContractNoWP()->funding==1||($project->lastContractNoWP()->type=="PPA" && strlen($project->lastContractNoWP()->signed)>0))
                                            <li>
                                                <form method="POST" action="{{url("payment")}}" id="paymentForm_{{$project->id}}">
                                        ...
                                        @else
                                        <li>
                                            <a href="{{route('launch/signBeforePaid',$project->id)}}" class="linkMakePaymentProject" data-id="{{$project->id}}"><i class="fa fa-money"></i>{{$project->ideaName}}</a>
                                        </li>
                                        @endif
                                    @endif
                                @endif
                    ...
                    @endif
                    @if(isset($allowTickets)  && $allowTickets == 1)
                    <li>
                        <a href="{{route('showTickets')}}"><i class="fa fa-envelope"></i>Manage Tickets</a>
                    </li>
                    @endif
                    <li>
                   ...
                   <li>
                        ...
                        <ul class="nav nav-second-level">
                            <li id="changePass" data-id="{{$client->id}}">
    ...
    @include('omi.tools.client_services.returnPatentAppNotesModal')
    ...
    <script src="{{asset("/js/omi/launch.js")}}"></script>
