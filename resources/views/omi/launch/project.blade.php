...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                ...
                @endif
                @if($letterOfEngagement)
                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <a class="dashboard-stat dashboard-stat-light blue-soft" href="{{route('launch/signLetterOfEngagement',$project->id)}}">
                            <div class="visual">
                                <i class="fa fa-pencil"></i>
                            </div>
                            <div class="details">
                ...
                @endif
                    {{--This "if" checks if exist some docs to sign
                        1st condition checks the client service exist, 2nd condition checks for several docs:
                        *docsToSign=>Patent App(Dec, Additional Dec, POA, MicroEntity), Agreement Ext
                        *The patent app was sent and still dont approved
                        *upgLetterSign=> pct epo upgrade was sent and still dont signed
                        *upgLetterPCTSign=> pct upgrade was sent and still dont signed or
                                            pct psa was sent and still dont signed
                        *ilcNeedSign ilc patented contract was sent and still dont signed or
                                     marketing agreement sent and still dont signed, neither the realse form
                        *provInvoiceSign=>the provisional invoice was sent and still dont singed --}}
                @if(($cS=$project->clientServicesProject())!=null && ($docsToSign>0 || ($cS->appSent_created_at!="" && $cS->patentAppApproved_created_at == "") || ($cS->appSentD_created_at != "0000-00-00 00:00:00" && $cS->patentAppApprovedD_created_at == "0000-00-00 00:00:00") || $upLetterSign == 1 || $upgLetterPCTSign == 1 || $ilcNeedSign == 1 || $provInvoiceSign == 1))
                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <div id="showDocsDiv" class="dashboard-stat dashboard-stat-light blue-soft" style="cursor:pointer;" data-pid="{{$project->id}}" >
                            ...
                            <div class="details">
                                ...
                                <input type="hidden" name="document" value="{{$upgLetterPCTSign}}">{{--esto se usa pa algo?--}}
                            </div>
                        </div>
                    </div>
                @endif
                @if($allowPayment)
                    ...
                    @else
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <a class="dashboard-stat dashboard-stat-light green-soft" href="{{route('launch/signBeforePaid',$project->id)}}">
                                <div class="visual"><i class="fa fa-money"></i></div>
                                <div class="details">
                                    <div class="number">
                                        Make a Payment
                ...
                @else
                    @if($allowPpaPlans)
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <a class="dashboard-stat dashboard-stat-light green-turquoise" id="addPlan" data-id="{{$project->id}}">
                                <div class="visual"><i class="fa fa-money"></i></div>
                                <div class="details">
                                    <div class="number">
                                        Plan Add-ons
            ...
            <div class="col-md-9 col-sm-12 col-xs-12">
                <ul class="nav nav-tabs" role="tablist">
                    <div class="caption">
                        <span class="projectStatus">PROJECT STATUS: {{$project->lastRequestToAdmin() && strlen($project->lastRequestToAdmin()->adminStage)>0?$project->lastRequestToAdmin()->adminStage:"PRELIMINARY SCREENING"}}</span>
                    </div>
                    ...
                    @endif
                    @if($pcs!=null  && $pcs->id=="154" && $ilcInfo['exist'] == 1)
                        <li>
                    ...
                    <div class="tab-content col-md-12">
                        ...
                        <div class="tab-pane" id="portlet_tab_2"  role="tabpanel">
                            <div class="form-group col-md-6 col-sm-12 col-xs-12">
                                ...
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-user"></i></span>
                                    <input type="text" id="coInventor" class="form-control" value="{{ $project->coInventor}}" disabled="disabled">
                                </div>
                            </div>
                            <div class="form-group col-md-6 col-sm-12 col-xs-12">
 ...
 Your patent was filed on {{date('m/d/Y',strtotime($pcs->patentAppFiled_created_at))}} -Application number is {{$pcs->patentAppNo}}"></i>
                        ...
                        @endif
                        @if($pcs!=null  && $ilcInfo['exist'] == 1)
                            <div class="tab-pane" id="portlet_tab_4"  role="tabpanel">
                                @if($ilcInfo['contractDate'] != -1)
                                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                        <label class="col-md-9">THE AGREEMENT START ON: </label>
                                        <div class="col-md-3">
                                            <p>{{$ilcInfo['contractDate']}}</p>
                                        </div>
                                    </div>
                                @endif
                                @if($ilcInfo['welcomePkcg'] != -1)
                                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                        <label class="col-md-9">YOU RECEIVED A WELCOME PACKAGE ON: @if($ilcInfo['welcomePackEmail'] != -1)&nbsp;&nbsp;&nbsp;&nbsp;(<a href="{{url($ilcInfo['welcomePackEmail'])}}" target='_blank'>PDF</a>) @endif</label>
                                        <div class="col-md-3">
                                            <p>{{$ilcInfo['welcomePkcg']}}</p>
                                        </div>
                                    </div>
                                @endif
                                @if($ilcInfo['introCall'] != -1)
                                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                        <label class="col-md-9">YOU RECEIVED AN INTRO CALL ON: </label>
                                        <div class="col-md-3">
                                            <p>{{$ilcInfo['introCall']}}</p>
                                        </div>
                                    </div>
                                @endif
                                @if($ilcInfo['introCallEmail'] != -1)
                                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                        <label class="col-md-9">YOU RECEIVED INTRO EMAIL ON:@if($ilcInfo['introCallILCEmail'] != -1)&nbsp;&nbsp;&nbsp;&nbsp;(<a href="{{url($ilcInfo['introCallILCEmail'])}}" target='_blank'>PDF</a>) @endif</label>
                                        <div class="col-md-3">
                                            <p>{{$ilcInfo['introCallEmail']}}</p>
                                        </div>
                                    </div>
                                @endif
                                @if($ilcInfo['courtesyUpdate'] != -1)
                                    @foreach($ilcInfo['courtesyUpdate'] as $courtesy)
                                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                            <label class="col-md-9">YOU RECEIVED AN UPDATE EMAIL ON: @if($courtesy[1] != -1)&nbsp;&nbsp;&nbsp;&nbsp;(<a href="{{url($courtesy[1])}}" target='_blank'>PDF</a>) @endif</label>{{----}}
                                            <div class="col-md-3">
                                                <p>{{$courtesy[0]}}</p>
                                            </div>
                                        </div>
                                ...
                                @endif
                                    ...
                                    @endif
                                    @if($ilcInfo['subEmail'] != -1)
                                        @foreach($ilcInfo['subEmail'] as $sub)
                                            <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                                <label class="col-md-9">A SUBMISSION EMAIL WAS SENT TO MANUFACTURER ON:@if($sub[1] != -1)&nbsp;&nbsp;&nbsp;&nbsp;(<a href="{{url($sub[1])}}" target='_blank'>PDF</a>) @endif </label>
                                                <div class="col-md-3">
                                                    <p>{{$sub[0]}}</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    @endif
                                    @if($ilcInfo['separationDate'] != -1)
                                        <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                            <label class="col-md-9">{{$ilcInfo['separationType']}} @if($ilcInfo['separationFile'] != -1)&nbsp;&nbsp;&nbsp;&nbsp;(<a href="{{url($ilcInfo['separationFile'])}}" target='_blank'>PDF</a>) @endif</label>
                                            <div class="col-md-3">
                                                <p>{{$ilcInfo['separationDate']}}</p>
                                            </div>
                                        </div>
                                    @endif
                            </div>
            ...
            <div class="col-md-3 col-sm-12 col-xs-12 " id="fileList_{{$project->id}}">
                ...
                @foreach($project->filesByType(array('public','clientVendor')) as $file)
                    <div class="col-md-12 col-sm-12 col-xs-12" id="divFile_{{$file->id}}"><span title="{{$file->fileName}}&nbsp;&nbsp;&nbsp;{{$file->created_at}}"><a href="{{asset("/".$file->url)}}" target="_blank">{{strlen($file->fileName)>25?substr($file->fileName,0,23).'...':$file->fileName}}</a></span></div>
                @endforeach
            </div>
        </div>
    </div>
    @include('omi.tools.addPpaPlanModal')
    @include('omi.launch.client_services.editCoInvModal')
    @include('omi.launch.client_services.clientEmailModal')
